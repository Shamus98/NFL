    ...
    if ret:
        ...
        for m,n in matches:
            if m.distance < 0.7*n.distance:
                ...
                for j in range(nj):
                    for k in range(nk):
                        if (kp[count][j,k, 0] + 30 >= proverka[0]) and (kp[count][j,k, 0] - 30 <= proverka[0])and (kp[count][j,k, 1] + 30 >= proverka[1]) and (kp[count][j,k, 1] - 30 <= proverka[1]):
                            proverka_bool = True
                            break
        ...
        for i, Rt in enumerate(zip(Rs, Ts)):
            ...
            if i==3:
                if count==0:
                    R1 = np.array(R)
                    R_inv = np.linalg.inv(R1)
                    matrixrotates = R_inv*(-1)*np.array(t)
                else:
                    R1 = np.array(R)
                    R_inv = np.linalg.inv(R1)
                    matrixrotates = matrixrotates*R_inv*(-1)*np.array(t)
            print('ts=')
            print(t)
        ...
        axarr.scatter(dst_pts[:,:,0], dst_pts[:,:,1],
           c = 'deeppink', s = 1)
        axarr.imshow(prev_frame)
        #plt.title('{0} {1}'.format(vectorot[0],vectorot[1]))
        # f.savefig('/Users/vanurin/Desktop/Kaggle/NFL/videoframe2/{:d}.png'.format(count))
